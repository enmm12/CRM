...
<script type="text/javascript">
	...
	$(function(){
		...
		//关联线索和市场活动的模态窗口的搜索框绑定键盘按下事件，当为回车时执行模糊查询，
		$("#aname").keydown(function (event) {

			if(event.keyCode == 13){

				//回车按下，根据信息模糊查询市场活动，将所有市场活动除了已关联的外都展现

				$.ajax({

					url : "workbench/clue/getActivityListByAnameAndNotByClueId.do",
					data : {
				...
				$.ajax({
					...
					success : function(data){
						...
						if(data.success){
							...
							//清空模态窗口中的数据,搜索框中的信息，选择框中的√
							$("#aname").val("");
							$("input[name=xz]:checked").prop("checked",false);
							$("#activitySearchBody").text("");
...
<body>
	...
	<div style="position: relative; top: -70px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 10px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>${c.appellation}</b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
